...
When every bullet is structured exactly the same, it can lull your readers into believing they know what's coming, which makes them lose focus. Big mistake!
...
This example shows the exact same structure among all the bullets. Notice how you start to scan part-way through.
Here's how we could vary their structure to add interest (and create more curiosity by teasing the solution instead of sharing it outright):
...
Notice that the structure and length of each bullet changes. That creates interest and keeps people reading. Simple formatting tricks, like adding spacing between bullets or bolding alternative lines, can also improve readability.
