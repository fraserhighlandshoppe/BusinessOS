# Content from Using BuzzSumo.md
...
Using BuzzSumo
BuzzSumo is a tool that tells you what is popular and what is getting shared the most on several major social media sites, including Facebook, Twitter, LinkedIn, Pinterest and Google+. You can use the free version of BuzzSumo to help you determine good potential Lead Magnets and Tripwires as well as potential Core Offers and Profit Maximizers. Here’s how.
First off, point your browser to BuzzSumo.com. You should see a screen similar to the one in the image below.
Using the numbered red circles for guidance, you can see that in number 1 I have already typed in our primary keyword, “memory improvement” and generated the top results. This is all available with the free version of BuzzSumo.
Number 2 shows that we are sorting based on posts for the past year. Number 3 allows you to choose the language you want to search for. In this case we chose English. Number 4 allows you to choose the country in which you want to search for posts. In this case, we choose the USA.
Number 5 is a huge help in finding specific types of things that may be of interest to our audience. More on that in just a bit, but for now, we’ve selected all content types to search through.
Number 6 shows that we can sort the results by the total number of shares or by the number of shares on any of the five major social networks that BuzzSumo supports.
Finally, number 7 shows the articles that have received the greatest number of shares by whatever means you sorted.
For our purposes, you can see that some of the top shared content is about eating chocolate, sniffing rosemary and eating the right foods. Armed with this knowledge of what people like and like to share, we may want to think about a Lead Magnet that provides information about Chocolate or Rosemary for memory improvement.
Or, we may want to consider a Tripwire that provides memory improving chocolate or sniffable rosemary herbs or oils.
Now, do this with your primary keyword(s) and make notes based on what you find on your BuzzSumo Research Ideas Worksheet.
If you’re looking specifically for giveaway (AKA Lead Magnet ideas), you can also just untick all the boxes in the “Content Types” section and BuzzSumo will tell you all the articles that were shared the most involving giveaways. This can sometimes provide great Lead Magnet ideas.
Let’s do that now. The image below shows the results of searching for only giveaway content:
Sure enough, there is one giveaway related bit of content, but it only received a total of 9 shares across all five social media networks, so it doesn’t seem conclusive enough to give us guidance here. Try this for your keywords and see if you can get any helpful information.
