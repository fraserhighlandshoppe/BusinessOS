...
JSON Ads
The JSON (which stands for JavaScript Object Notation) growth tool enables you to start Messenger conversations with users who click on your Facebook ad. It works hand-in-hand with click-to-messenger ads, like this one:
You don’t HAVE to use JSON in conjunction with click to Messenger ads—you could create a click-to-messenger ad using the messaging options that are native to the Facebook ads manager. But the JSON tool gives you some cool functionality that can help make those ads even more effective.
And 1 big benefit of using JSON ads is that they allow you to bypass the default Get Started button that Facebook usually displays when someone clicks on an ad that opens up Messenger.
So, instead of showing the Get Started button, JSON allows you to go right into your opt-in message.
Here’s a simple flow showing how this could work:
And here’s a quick walkthrough of how you can set this up inside the Facebook Ads Manager. For this example, we’re going to choose the Traffic objective:
...
A window will show up with a bunch of code inside it. You want to highlight that code, delete it, and replace it with the JSON code from your Messenger marketing platform (like ManyChat):
Hit Confirm, and that’s it! Your ad will now automatically go straight to your opt-in message when users click on your call to action button.
This is really a no-brainer if you’re running click-to-Messenger ads, since it cuts down on the number of clicks required from your users.
